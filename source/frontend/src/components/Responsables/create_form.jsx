...
const CreateForm = ({ trigger, setTrigger, setRefresh }) => {
    ...
    /* Handles the form, passes the data to backend and creates a responsable */
    function handleForm(event) {
        event.preventDefault()
        let responsable = {
            Nombre: Nombre
        }
        fetch('/backend/responsables/create/guardar', {
            method: 'POST',
        ...
        .then(function (body) {
            console.log(body);
            setFormStatus({ success: true, message: 'El responsable ha sido creado de forma exitosa.' });
        })
        .catch(function (error) {
            console.error('There has been a problem with your fetch operation:', error);
            setFormStatus({ success: false, message: 'Se produjo un error y no se pudo crear el nuevo responsable.' });
        });
    }
